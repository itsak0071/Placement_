...
struct Node1 {
	ll val; // may change/// node is storing only one value
	Node1() { // Identity element
		val =0 ;	// may change//agr overlap nhi huwa to kya return karoge(ie..defalut value)
	}
	Node1(ll p1) {  // Actual Node
		val = p1; // may change///   you want to store initiallly leaf node pe a[i]  sotre krna chahte ho
	}
...
struct Update1 {
	...
	Update1(ll p1) { // Actual Update
		val = p1; // may change   you want to make  update 
	}
	void apply(Node1 &a) { // apply update to given node
		a.val = val; // may change   a[i]=val;
	}
};
void solve(){
    int n; cin>>n ;
