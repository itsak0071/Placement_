...
/*--------------------------------------------------------------------------------------------------------------------------*/
   ...
   struct Node1 {
   	ll min1 ,max1; // may change  (node me kon kon si values store kroge)
   	Node1() {
   	min1=1e9;
   	 max1=-1e9; // Identity element
   	    	// may change  (agr overlap nhi hiwa to defalut value  min  max  ke liye kya return kroge)
   	}
   	Node1(ll p1) {  // Actual Node
   		min1 = p1; //(array se hume aik value  mili p1    to  hum segmwnt tree ke leaf node pr kya  kya store krenge) 
   		max1=p1;
   		
   	}
   	void merge(Node1 &l, Node1 &r) { // Merge two child nodes
   		min1= min(l.min1, r.min1);
   		max1=max(r.max1, l.max1);
       ...
       int q; cin>>q;
        while(q--){
           ...
           if(type==1){
               ...
               int l, r; 
               cin>>l>>r;
               cout<<sg.make_query(l,r).min1<<" "<<sg.make_query(l,r).max1<<endl;
           }
        }
   }
